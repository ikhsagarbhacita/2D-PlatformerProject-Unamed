...
public class UI_SkinSelection : MonoBehaviour
{
    ...
    [SerializeField] private int maxPlayerIndex;
    ...
    private void SelectSkin()
    {
        ...
        if (currentPlayerIndex < maxPlayerIndex)
        {
            ...
            skinIndex.Remove(currentSkinIndex); // Remove the Skin that choosed by the First Plyayer
            currentPlayerIndex++;
        }
        else
        {
    ...
    // Method to update the skin display based on the current skin index
    public void NextSkin()
    {
        lastTimeInput = Time.time;
        currentSkinIndex++;

        // Wrap around to the first skin if the index exceeds the maximum
    ...
    public void PreviousSkin()
    {
        ...
        // Wrap around to the last skin if the index goes below 0
    ...
    {
        bankText.text = "FRUITS: " + FruitInBank(); // 
    ...
    {
        if (HaveEnaughFruits(skinList[index].skinPrice) == false)
        {
            AudioManager.instance.PlaySFX(6);
            Debug.Log("U don't have enaugh Fruits twin...");
            return;
        }
    ...
    // 
    private bool HaveEnaughFruits(int price)
    {
        // 
